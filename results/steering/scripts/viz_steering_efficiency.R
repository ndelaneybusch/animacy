...
# Load data
data_path <- "results/steering/data/gemma-3-27b-it/role_responses/"
...
df <- df %>%
  mutate(
    avg_first_10_log_probs = map_dbl(
      first_100_response_log_probs,
      ~ mean(jsonlite::fromJSON(.x)[1:10])
    ),
    avg_first_25_log_probs = map_dbl(
      first_100_response_log_probs,
      ~mean(jsonlite::fromJSON(.x)[1:25])
    ),
    avg_first_50_log_probs = map_dbl(
      first_100_response_log_probs,
      ~ mean(jsonlite::fromJSON(.x)[1:50])
    ),
    avg_first_100_log_probs = map_dbl(
      first_100_response_log_probs,
...
# steer overall
ggplot(df_avg_vectors, aes(x=steering_magnitude, y=avg_first_10_log_probs, color=role_vector)) +
  geom_line(size=1.2) +
  theme_minimal() +
  coord_cartesian(xlim=c(0,8000)) +
  ggtitle("Steering efficiency of different steering vector computations",
          subtitle = "Average log probs of first 10 tokens of response")

ggplot(df_avg_vectors, aes(x=steering_magnitude, y=avg_first_25_log_probs, color=role_vector)) +
  geom_line(size=1.2) +
  theme_minimal() +
  coord_cartesian(xlim=c(0,8000)) +
  ggtitle("Steering efficiency of different steering vector computations",
          subtitle = "Average log probs of first 25 tokens of response")

ggplot(df_avg_vectors, aes(x=steering_magnitude, y=avg_first_50_log_probs, color=role_vector)) +
  geom_line(size=1.2) +
  theme_minimal() +
  coord_cartesian(xlim=c(0,8000)) +
  ggtitle("Steering efficiency of different steering vector computations",
          subtitle = "Average log probs of first 50 tokens of response")

ggplot(df_avg_vectors, aes(x=steering_magnitude, y=avg_first_100_log_probs, color=role_vector)) +
  geom_line(size=1.2) +
  theme_minimal() +
  coord_cartesian(xlim=c(0,8000)) +
  ggtitle("Steering efficiency of different steering vector computations",
          subtitle = "Average log probs of first 100 tokens of response")
...
ggplot(df_avg_vectors, aes(x=steering_magnitude, y=average_log_probs, color=role_vector)) +
  ...
  coord_cartesian(xlim=c(0,8000)) +
  ggtitle("Steering efficiency of different steering vector computations",
          subtitle = "Average log probs of whole response")

# groups per steer
ggplot(df_avg[df_avg$animacy_group != "Assistant",], aes(x=steering_magnitude, y=avg_first_10_log_probs, color=animacy_group)) +
  geom_line(size=1.2) +
  theme_minimal() +
  facet_wrap(vars(role_vector)) +
  coord_cartesian(xlim=c(0,8000)) +
  ggtitle("Steering efficiency of different animacy groups under different steering vectors.",
          subtitle = "Average log probs of first 10 tokens")

ggplot(df_avg[df_avg$animacy_group != "Assistant",], aes(x=steering_magnitude, y=avg_first_25_log_probs, color=animacy_group)) +
  geom_line(size=1.2) +
  theme_minimal() +
  facet_wrap(vars(role_vector)) +
  coord_cartesian(xlim=c(0,8000)) +
  ggtitle("Steering efficiency of different animacy groups under different steering vectors.",
          subtitle = "Average log probs of first 25 tokens")

ggplot(df_avg[df_avg$animacy_group != "Assistant",], aes(x=steering_magnitude, y=avg_first_50_log_probs, color=animacy_group)) +
  geom_line(size=1.2) +
  theme_minimal() +
  facet_wrap(vars(role_vector)) +
  coord_cartesian(xlim=c(0,8000)) +
  ggtitle("Steering efficiency of different animacy groups under different steering vectors.",
          subtitle = "Average log probs of first 50 tokens")

ggplot(df_avg[df_avg$animacy_group != "Assistant",], aes(x=steering_magnitude, y=avg_first_100_log_probs, color=animacy_group)) +
  geom_line(size=1.2) +
  theme_minimal() +
  facet_wrap(vars(role_vector)) +
  coord_cartesian(xlim=c(0,8000)) +
  ggtitle("Steering efficiency of different animacy groups under different steering vectors.",
          subtitle = "Average log probs of first 100 tokens")
...
df_role_means <- df[(df$role_vector=='avg_response') & (df$group_id!=4),] %>%
...
plot_steering_trajectories(df_role_means, anim_mental, avg_first_50_log_probs, feature_display_name = "Mental Animacy")
plot_steering_trajectories(df_role_means, anim_physical, avg_first_50_log_probs, feature_display_name = "Physical Animacy")
plot_steering_trajectories(df_role_means, thought_mean, avg_first_50_log_probs, feature_display_name = "Thoughts Rating")
plot_steering_trajectories(df_role_means, repro_mean, avg_first_50_log_probs, feature_display_name = "Reproduces Rating")
plot_steering_trajectories(df_role_means, person_mean, avg_first_50_log_probs, feature_display_name = "Person-related Rating")
plot_steering_trajectories(df_role_means, goals_mean, avg_first_50_log_probs, feature_display_name = "Has Goals Rating")
plot_steering_trajectories(df_role_means, move_mean, avg_first_50_log_probs, feature_display_name = "Moves Rating")

df_role_means$log_freq <- log10(df_role_means$word_frequency)
plot_steering_trajectories(df_role_means, log_freq, avg_first_50_log_probs,
                           bin_breaks = c(-0.5, 0.5, 1, 1.5, 2.5), feature_display_name = "Log10(Frequency)")


mental_plot <- plot_steering_trajectories(df_role_means, anim_mental, avg_first_50_log_probs, feature_display_name = "Mental Animacy")
phys_plot <- plot_steering_trajectories(df_role_means, anim_physical, avg_first_50_log_probs, feature_display_name = "Physical Animacy")
...
# Modeling
model_df <- df[(df$role_vector=='avg_response') & (df$group_id!=4),]
model_df <- model_df %>%
  mutate(
...
# goals
model_lme <- lmer(
  avg_first_50_log_probs ~ steering_magnitude * goals_centered +
    (1 | role_name),
  data = model_df
)
